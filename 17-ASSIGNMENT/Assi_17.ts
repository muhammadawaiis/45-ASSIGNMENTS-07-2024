let biggerDinerTable: string[] = ["Kinza", "Tooba", "Iqra"];
for (let i = 0; i < biggerDinerTable.length; i++) {
  console.log(
    `Dear! ${biggerDinerTable[i]}, ,please on My Big Dinner I would be thankfull`
  );
}
...
biggerDinerTable.splice(
  Math.floor(biggerDinerTable.length / 2),
  0,
  "Hasnain Sher"
);
...
while (biggerDinerTable.length > 2) {
  ...
  console.log(
    `Dear! ${removedlist} ,Sorry Have less Table ,ane  sorry you can’t invite you to dinner.`
  );
}
//Conformation
...
for (let i = 0; i < biggerDinerTable.length; i++) {
  console.log(
    `Dear! ${biggerDinerTable[i]}, You are still invite for Dinner pleace come on time`
  );
}
console.log(
